    ...
    <div class="row">
        <div class="col-md-10 col-sm-6 col-xs-12">
            ...
            <div class="panel-body">
                <form class="form-horizontal" id="articleForm" method="post" action="/updateArticleById">
                    <input type="hidden" name="articleId" value="${article.articleId}">
                    <div class="form-group">
                        <label>文章标题:</label>
                        <input class="form-control" id="article_title" value="${article.articleTitle}" name="articleTitle" type="text">
                    ...
                    <div class="form-group">
                        ...
                        <!-- 加载ueditor编辑器的容器 -->
                        <script id="container" id="article_content" name="articleContent" type="text/plain">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-inline" name="articleParentCategoryId" id="articleParentCategoryId" style="width: 120px;height: 40px;">
                            ...
                            <c:forEach items="${cats}" var="cat">
                                <c:if test="${cat.categoryPid==0}">
                                    ...
                                    <c:if test="${article.articleParentCategoryId==cat.categoryId}">
                                        selected
                                    </c:if>
                        ...
                        <select class="form-inline" name="articleChildCategoryId" id="articleChildCategoryId" style="width: 120px;height: 40px;">
                            <c:forEach items="${cats}" var="c">
                                <c:if test="${c.categoryPid==article.articleParentCategoryId}">
                                    <option value="${c.categoryId}" <c:if test="${article.articleChildCategoryId==c.categoryId}">selected</c:if>>${c.categoryName}</option>
                                </c:if>
                            </c:forEach>
                        </select>
                    ...
                    <div class="form-group">
                        <label>标签:</label>
                        <c:set var="tagIds" value="${fn:split(article.articleTagIds,',')}"/>
                        <c:forEach items="${tags}" var="tag">
                            <label class="checkbox-inline">
                                <input type="checkbox" name="articleTagIds" value="${tag.tagId}" title="${tag.tagName}"
                                <c:forEach items="${tagIds}" var="id">
                                    <c:if test="${tag.tagId==id}">checked</c:if>
                    ...
                    <div class="form-group">
                        ...
                        <label class="radio-inline">
                            <input type="radio" name="articleStatus" id="inlineRadio1" <c:if test="${article.articleStatus==1}">checked</c:if> value="1">发布
                        </label>
                        <label class="radio-inline">
                            <input type="radio" name="articleStatus" id="inlineRadio2" <c:if test="${article.articleStatus==0}">checked</c:if> value="0">草稿
                        </label>
                    </div>
